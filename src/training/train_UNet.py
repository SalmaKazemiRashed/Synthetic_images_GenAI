...
class SaveMetrics(Callback):
    def on_epoch_end(self, epoch, logs=None):
        with open('../results/UNet_gray.txt', 'a') as f:
...
with tf.device('cpu:0'):
    ...
    val_dataset = Preprocessing.load_dataset(val_nuclei_dir, val_cell_dir)
...
checkpoint_path = "../models/UNet/UNet_gray_{epoch:04d}_{val_loss:.4f}.h5"
...
generator.fit(train_dataset, validation_data=val_dataset, epochs=20, callbacks=[checkpoint_callback, save_metrics_callback])
generator.save('../models/UNet/UNet_gray_final_generator.h5')
